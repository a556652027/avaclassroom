//  (原 www/modules/app.module.login.js 原封搬移)
...
// DESC : 使用者登入
//===============================================================================
export function CsRequestLogin(member_cid, password, callback, abortController) {
      ...
      if (json_object.errno == '1') {
        ...
        if (json_object.owned_products) {
          window.sessionStorage.setItem(
            'owned_products',
            JSON.stringify(json_object.owned_products),
          )
          window.sessionStorage.setItem(
            'default_product',
            json_object.default_product,
          )
        }
      } else {
...
// DESC : 使用者登出
//===============================================================================
export function CsRequestLogout() {
  if (window.Cyberspace && window.Cyberspace.Client) {
    Cyberspace.Client.clsSession()
  }
}
...
// DESC : 重設密碼
//===============================================================================
export function CsRequestResetPassword(member_cid, email, callback, abortController) {
  if (!window.Cyberspace || !window.Cyberspace.Client) {
    if (callback) callback(new Error('Network Client not ready'), null)
    return
  }
...
//===============================================================================
export function CsRequestVerifyResetToken(reset_token, callback, abortController) {
  if (!window.Cyberspace || !window.Cyberspace.Client) {
    if (callback) callback(new Error('Network Client not ready'), null)
    return
  }
