...
function receiveAllNumOfIngredientNotice(callback) {
    Cyberspace.Client.SyncRequest("receivce_all_ingredient_notice", { sessionid: Cyberspace.Client.GetSessionUid() }, function (result) {
        result = StringToJson(result);
        if (result.errno === "1") {
            if (callback) {
                let ret = {};
                ret["notice_of_edit_ingredient_list"] = {};
                ret["notice_of_edit_ingredient_list"]["2"] = 0;
                ret["notice_of_edit_ingredient_list"]["3"] = 0;
                result.notice_of_edit_ingredient.forEach((edit_ingredient) => {
                    if ([1, 2].includes(parseInt(edit_ingredient[1]))) {
                        ret["notice_of_edit_ingredient_list"]["2"] += parseInt(edit_ingredient[0]);
                    } else {
                        ret["notice_of_edit_ingredient_list"][edit_ingredient[1]] += parseInt(edit_ingredient[0]);
                    }
                });
                ret["notice_of_proudct_ingredient_list"] = {};
                result.notice_of_proudct_ingredient.forEach((proudct_ingredient) => {
                    if ([1, 2].includes(parseInt(proudct_ingredient[3]))) {
                        if (typeof ret["notice_of_proudct_ingredient_list"]["2"] === "undefined") {
                            ret["notice_of_proudct_ingredient_list"]["2"] = {};
                        }

                        if (typeof ret["notice_of_proudct_ingredient_list"]["2"][proudct_ingredient[2]] === "undefined") {
                            ret["notice_of_proudct_ingredient_list"]["2"][proudct_ingredient[2]] = {
                                number: parseInt(proudct_ingredient[0]),
                                supplier_alias: proudct_ingredient[1],
                            };
                        } else {
                            ret["notice_of_proudct_ingredient_list"]["2"][proudct_ingredient[2]].number += parseInt(proudct_ingredient[0]);
                        }
                    } else if (parseInt(proudct_ingredient[3]) === 4) {
                        if (typeof ret["notice_of_proudct_ingredient_list"]["4"] === "undefined") {
                            ret["notice_of_proudct_ingredient_list"]["4"] = {};
                        }

                        ret["notice_of_proudct_ingredient_list"]["4"][proudct_ingredient[2]] = {
                            number: parseInt(proudct_ingredient[0]),
                            supplier_alias: proudct_ingredient[1],
                        };
                    } else {
                        if (typeof ret["notice_of_proudct_ingredient_list"]["3"] === "undefined") {
                            ret["notice_of_proudct_ingredient_list"]["3"] = {};
                        }

                        ret["notice_of_proudct_ingredient_list"]["3"][proudct_ingredient[2]] = {
                            number: parseInt(proudct_ingredient[0]),
                            supplier_alias: proudct_ingredient[1],
                        };
                    }
...
function Fee_List(callback) {
    Cyberspace.Client.Request(
        "fee/list",
        {
            ...
            supplier_uid: 0,
            record_state: 0,
            field_list: "supplier_uid,creater_id,processor_id,fee_status_id,Approved_id",
            is_self_only: 1,
        },
        function (result) {
            result = StringToJson(result);
...
// 找出所有訂單資料，用於退貨
function Return_Request_Select_Order_All(supplier_uid, prn_record_state, begin_date, end_date, limit_offset, limit_rowcount, whereClause, orderby, date_type, license_type, callback, is_group_buy = "") {
    Cyberspace.Client.Request(
...
function Return_Request_Select_005(supplier_uid, order_nid, begin_date, callback) {
    Cyberspace.Client.Request(
        "return_select_005",
        {
            sessionid: Cyberspace.Client.GetSessionUid(),
...
function Return_Request_Select_Count(supplier_uid, prn_record_state, start_date, begin_date, end_date, whereClause, date_type, license_type, callback, is_group_buy = "") {
    Cyberspace.Client.Request(
        "return_select_count",
        {
            sessionid: Cyberspace.Client.GetSessionUid(),
...
function Get_Supplier_Uid_Alias(supplier_obj) {
    if (supplier_obj) return padLeft(supplier_obj.supplier_uid, 3) + "_" + supplier_obj.supplier_alias;
    return "";
}
...
function Supplier_Request_Select_Show(callback) {
    Cyberspace.Client.Request(
        "supplier/list",
        {
            ...
            field_list: "supplier_uid,supplier_code,supplier_alias,supplier_name,supplier_type,supplier_phone,supplier_address,commission_rate,shopping_system,record_state,contractor_a,account_information,winton_name,contractor_b,supplier_country",
        },
        function (result) {
            result = StringToJson(result);
...
function PageReport_Request(supplier_uid, report_nid, byyyy, bmm, bdd, eyyyy, emm, edd, callback) {
    Cyberspace.Client.Request(
        "pagereporter",
        {
            sessionid: Cyberspace.Client.GetSessionUid(),
